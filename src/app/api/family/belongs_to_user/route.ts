...
import { UserService, ListFamilyUsersResponse, ListFamilyUsersRequestSchema } from "@/gen/user_pb";
import { fromJson } from "@bufbuild/protobuf";
import { createClient } from "@connectrpc/connect";
import { createParamsFromUrl, doAuthRequest } from "@utils/request";


export type UserApiResponse = { ok: true, data: ListFamilyUsersResponse } | { ok: false, error: Error };
...
const doGrpcRequest = async (req: Request, _method: string, accessToken: string) => {
    ...
    const request = fromJson(ListFamilyUsersRequestSchema, requestJson);
    return client.listFamilyUsers(request);
}
